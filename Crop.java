...
public abstract class Crop {
    ...
    public GrowthStage getStage(){return this.stage;}
    ...
    public void grow(){
        ...
        if (state == GrowthStage.values().length-1 || state == GrowthStage.values().length-2){
            return;
        }
    ...
    public boolean isReady(){
        return stage.equals(GrowthStage.MATURE);
    }

    public abstract int getYield();
    public abstract String getType();

    public int getMarketprice(){return this.marketPrice;}

    protected int appropriateEventYield(int usualYield){
        if (this.fEvent.equals(FarmEvent.FRUTFUL_HARVEST)){
            return usualYield*2;
