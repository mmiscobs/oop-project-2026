...
public class SelfDescribedXML implements SerializedBlob.Factory {
    public SelfDescribedXMLBlob fromMap(Map<String, SerializedBlob> m) {
        return new SelfDescribedXMLBlob(Type.OBJECT, m, null, null);
    }

    public SelfDescribedXMLBlob array(List<SerializedBlob> l) {
        return new SelfDescribedXMLBlob(Type.ARRAY, null, l, null);
    }

    public SelfDescribedXMLBlob string(String s) {
        return new SelfDescribedXMLBlob(Type.PRIMITIVE, null, null, s);
    }

    public SelfDescribedXMLBlob intValue(int i) {
        return new SelfDescribedXMLBlob(Type.PRIMITIVE, null, null, i);
    }

    public SelfDescribedXMLBlob doubleValue(double d) {
        return new SelfDescribedXMLBlob(Type.PRIMITIVE, null, null, d);
    }

    public SelfDescribedXMLBlob booleanValue(boolean d) {
        return new SelfDescribedXMLBlob(Type.PRIMITIVE, null, null, d);
    }

    public SelfDescribedXMLBlob nullValue() {
        return new SelfDescribedXMLBlob(Type.PRIMITIVE, null, null, null);
    }

    public SelfDescribedXMLBlob primitive(Object o) {
        ...
        return new SelfDescribedXMLBlob(Type.PRIMITIVE, null, null, o);
    }

    private static SelfDescribedXML singleton;

    public static SelfDescribedXML getInstance() {
        if (singleton == null)
            singleton = new SelfDescribedXML();
        return singleton;
    }

    private Element toEl(SerializedBlob blob, Document doc) {
        switch (blob.type()) {
            case OBJECT -> {
                Element el = doc.createElement("object");
                for (Map.Entry<String, SerializedBlob> e : blob.map().entrySet()) {
                    Element entry = doc.createElement("entry");
                    entry.setAttribute("key", e.getKey());
                    entry.appendChild(toEl(e.getValue(), doc));
            ...
            case ARRAY -> {
                ...
                for (SerializedBlob v : blob.array())
                    el.appendChild(toEl(v, doc));
                return el;
            }
            case PRIMITIVE -> {
                Element el = doc.createElement("primitive");
                Object primitive = blob.primitive();
                if (primitive == null) {
    ...
    public SelfDescribedXMLBlob parse(String xml) {
        try {
            ...
            throw new RuntimeException("Failed to parse SelfDescribedXMLBlob", e);
        }
    }

    private SelfDescribedXMLBlob from(Element el) {
    ...
    public static class SelfDescribedXMLBlob implements SerializedBlob {
        private final Type type;
        private Map<String, SerializedBlob> map;
        private final List<SerializedBlob> array;
        private final Object primitive;

        private SelfDescribedXMLBlob(Type t, Map<String, SerializedBlob> m, List<SerializedBlob> a, Object p) {
            this.type = t;
            this.map = m;
            this.array = a;
            this.primitive = p;
        ...
        public String toSerialized() {
            try {
                ...
                doc.appendChild(SelfDescribedXML.getInstance().toEl(this, doc));
