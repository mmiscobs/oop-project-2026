...
public class FarmGUI extends JFrame {
    ...
    private final Runnable onReturnToMain;
    ...
    public FarmGUI(int numPlots, int startMoney, Runnable onReturnToMain) {
        ...
        this.day = 1;
        this.onReturnToMain = onReturnToMain;
    ...
    private JPanel buildHeader() {
        ...
        p.add(tickBtn);
        JButton returnBtn = styledButton("🏠 Main Menu", BLUE_C);
    ...
    private void doTick() {
        ...
        boolean anyVanished = false;
        for (int i = 0; i < farm.getNumberOfPlots(); i++) {
            FarmPlot p = farm.getPlot(i);
            if (p.didCropJustVanish()) {
                log("💀 Plot " + i + ": the dead crop has rotted away!");
                anyVanished = true;
            } else if (p.getCrop() != null) {
                if (p.getCrop().getStage() == GrowthStage.DEAD) {
                    log("☠  Plot " + i + ": " + p.getCrop().getName() + " has DIED from neglect! Harvest it tomorrow or it's gone.");
                } else if (p.getCrop().isReady()) {
